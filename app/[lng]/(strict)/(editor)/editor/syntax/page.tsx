...
~~~plain:마크다운
![로고-32 width-32](${Logo.src})
![로고-64 width-64](${Logo.src})
![로고-128 width-128](${Logo.src})
~~~


| width-32           | width-64           | width-128          |
| :----------------: | :----------------: | :----------------: |
| ![로고-32 width-32](${Logo.src}) | ![로고-64 width-64](${Logo.src}) | ![로고-128 width-128](${Logo.src}) |

이미지 제목에 \`width-[px]\` 또는 \`height-[px]\` 문법을 사용하여 크기를 조정할 수 있습니다. 이미지의 크기는 최대 크기를 제한하는 방식으로 작동하여 아무리 크게 설정하여도 문서를 벗어날 수 없습니다.

아무런 설정 없이 사용할 경우 이미지는 항상 문서의 빈 공간을 가득 채우기 때문에 이미지가 차지하는 영역보다 작은 이미지를 사용하는 경우 계단 현상이 발생할 수 있습니다.

## 특수 문법: 유튜브 동영상

~~~plain:마크다운
@[jSJZu-HUi6E] @[jSJZu-HUi6E]
~~~

@[jSJZu-HUi6E] @[jSJZu-HUi6E]

동영상 ID를 \`@[]\` 마크다운의 대괄호 안쪽에 입력하여 문서에 유튜브 동영상을 삽입할 수 있습니다. 한 줄 안에 여러 개의 동영상이 있다면 큰 스크린에서 가로로 배치 됩니다.
...
모닥위키는 알림용 배너 문법을 지원합니다. \`!\` 문자를 시작으로 빈 줄을 만날때까지 알림으로 인식합니다. 첫 번째 줄은 제목이 되고 나머지 줄은 설명이 됩니다. 설명내 줄바꿈은 띄어쓰기를 2회를 끝으로 다음 줄을 입력하면 됩니다. 관리 편의상 알림에 대한 문단은 모두 느낌표로 시작하도록 하는 것이 좋습니다. 느낌표의 개수에 따라 알림의 수준이 결정됩니다.
