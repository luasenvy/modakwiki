  ...
  return (
    <main
      ...
      className={cn(
        ...
        { "md:max-w-[calc(100%_-_var(--sidebar-width))] transition-normal delay-200 ease-in-out": !isMobile && state === "expanded" },
        className
      )}
      {...props}
    >
                ...
                <BreadcrumbItem key={`breadcrumb-${i}`} className={cn("text-muted-foreground")}>
                  <BreadcrumbPage className={cn("truncate",{
                    ...
                    {(href && breadcrumbs.length - 1 > i) ? (
                      <Link href={href} className="text-muted-foreground hover:text-blue-600 dark:hover:text-blue-500 hover:underline">{title}</Link>
                    ) : (
                      title
                    )}
                  </BreadcrumbPage>
